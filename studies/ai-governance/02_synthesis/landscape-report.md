...
changelog: v0.2 folds in the two HBR full texts (operator pulled them); prior v0.1 in archive/
---
...
> **Where this sits now (2026-06-19):** this report is product 1 of 3 and stays a
> reusable reference. The downstream products it fed now exist: the gap analysis
> (`gap-analysis.md`) and the rewritten instruction (`03_instruction/instruction-v0.3.md`).
> This report is unchanged in substance; it is the standard the others were measured against.
...
(Update, v0.2: both Harvard Business Review pieces have now been read in full from
operator-provided PDFs; their claims below rest on full text, not abstracts.)
...
**A Harvard Business Review checklist frames how to build the program, and its
single most transferable idea is design-then-phase.** A responsible-AI program is
enterprise-wide governance, not per-project guardrails: it "defines the
enterprise-wide policy, governance structures, roles and responsibilities,
processes, and more that enable wide-scale deployment of AI." `[hbr-rai-checklist]`
The authors' core warning is that companies rush to implement before they finish
designing, with predictable results ("inefficient and difficult-to-scale efforts…
wasted resources; and slowed innovation"). Their rule: phase the rollout, but do
not take baby steps in the design; "the program should be designed as fully as
possible prior to implementation," because "completing design of an RAI program
requires far fewer resources than implementation." `[hbr-rai-checklist]` They offer
an eight-question readiness test (strategic objectives; values tied to concrete
procedures; metrics for the program itself, not just the models; trained oversight
people; the personnel to do the work; harmonization with cyber, privacy, and IT
programs; a strategic roadmap; an implementation playbook), and advise that if you
cannot answer yes to all eight, slow the rollout and finish the design.
`[hbr-rai-checklist]` One test worth lifting directly: are your stated values tied
to procedures, or left abstract? "Articulated… abstractly, the values are impossible
to implement." `[hbr-rai-checklist]`

**A second HBR piece, by the same lead author, supplies the agentic urgency.** It
argues that risk programs built for "narrow AI" rest on assumptions that break as
you move to generative and then agentic systems, and it reframes "agentic" as a
five-stage staircase rather than a yes/no property. See Section 3 for the staircase
and the six controls that get harder as you climb. `[hbr-agentic-risks]`
...
**First, a framing worth borrowing: "agentic" is a staircase, not a switch.** HBR's
Reid Blackman lays out five stages of rising complexity: (1) one model connected to
another, (2) a model wired to dozens of databases, narrow AIs, and the open
internet, (3) the same plus the ability to take digital actions, which is the point
it becomes "multi-model agentic AI," (4) the ability to talk to other agents inside
your organization, and (5) the ability to talk to agents outside it, which he calls
"a head-spinning quagmire of incalculable risk." `[hbr-agentic-risks]` His warning is
that six things get harder at each step up: deciding who runs which risk assessment
when; the human in the loop's ability to "wisely stand between system outputs and
impacts decreases drastically"; the weight on go/no-go decisions that "most
organizations lack… pre-deployment evaluation frameworks" to make; real-time
monitoring; minimally disruptive intervention; and continuous role-level training.
`[hbr-agentic-risks]` **Assessment:** the practical use of the staircase is to locate
a given askSage use case on it. A retrieve-and-draft assistant sits low; the moment
an agent can take an action or call another agent, it has climbed, and the controls
below must climb with it. That is the same logic as the OWASP maturity model: match
governance to the most advanced agent you actually run, or lower the deployment tier.
...
- **Both HBR pieces are now full-text (v0.2 upgrade).** The operator pulled the two
  Harvard Business Review articles; they were read in full from the PDFs, captured
  in `01_sources/hbr-rai-checklist.md` and `01_sources/hbr-agentic-risks.md` (PDFs
  preserved under `01_sources/hbr/`), and their claims above no longer rest on
  abstracts. `[hbr-rai-checklist]` `[hbr-agentic-risks]`
...
The two HBR items are now pulled and captured (see Section 6). What remains is the
McKinsey set, worth pulling with your subscription. URLs are exact in
`../source-ledger.md`.
...
*Done (no longer on the pull list): HBR, "Designing a Responsible AI Program? Start
with this Checklist" `[hbr-rai-checklist]` and "Organizations Aren't Ready for the
Risks of Agentic AI" `[hbr-agentic-risks]` — both captured in full.*
...
the two most likely gaps, on this evidence), and what is over-built. The HBR full
texts are now in; the remaining loose end is the McKinsey set, worth pulling so the
last of the corporate claims rest on full sources rather than abstracts.
