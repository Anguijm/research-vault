...
a. **AI output is authoritatively evaluated by the cognizant domain authority.** No AI
...
AI-produced content used in that department. That evaluator performs the evaluation for
content in the department's own domain and routes content that falls in another domain to
the cognizant authority in paragraph (a) — anything technical goes to the CHENG chain. The
designated evaluator is recorded with the department's use cases in the inventory
(Section 8).
...
technical scope per 8.c; CIO validates platform/security), **designate the department's
authoritative AI content evaluator (Section 6.b)**, enforce the human-in-the-loop and
least-privilege requirements, and submit their use cases to the inventory.
