...
title: SRF-JRMC askSage governance instruction — rewrite v0.2
status: draft v0.2.1 (post-logic-red-team; operator command-context folded)
classification: internal
created: 2026-06-19
derives_from: 02_synthesis/gap-analysis.md (Part F) + _decisions.md
red_team: 03_instruction/instruction-red-team.md
prior_version: 03_instruction/instruction-v0.1.md
...
v0.2 is v0.1 after a logic/consistency red-team. The substantive controls are the same;
this version removes an internal contradiction (records vs. spillage), closes a
cross-caveat data loophole for the Local National workforce, makes the PII/PHI and
verification controls enforceable rather than aspirational, reorders so the
authoritative knowledge base sits where it belongs, and states accountability in Navy
form. Bracketed markers flag the command's own decisions. Acronyms expanded on first use.
...
*[COMMAND DECISION — instance basis: written to govern the command's USE of askSage
whether the instance is command-standalone or provided through the DoD enterprise
platform GenAI.mil. If the latter, the Authority to Operate, vendor data-handling terms,
and platform-level monitoring/guardrails are inherited from the enterprise provider
rather than owned by the command; mark those paragraphs accordingly. Resolve before
promulgation.]*
...
up to the approved DoD Impact Level [IL4 / IL5 — confirm], including routine CUI and
Controlled Technical Information (CTI, marked CUI//SP-CTI). This covers most of the
command's maintenance, engineering, and administrative work.
...
"do not enter" rule is necessary but not sufficient. To the extent the platform
supports it, the CIO shall implement Data Loss Prevention (DLP) or sensitive-pattern
interception at the prompt-ingress layer to intercept PII, PHI, and prohibited-data
patterns before they reach the model, aligned with enterprise guardrails. Detection of
what gets through is covered by Continuous Monitoring (Section 9).
...
(Section 11.a). *[MAP TO COMMAND — name the cognizant SCG(s). COMMAND DECISION, optional
defense-in-depth: scoping an agent's default horizontal data reach below the user's full
access lowers inadvertent one-prompt compilation at a friction cost, but does not reduce
the underlying risk (the user could still compile manually) — keep or drop.]*
...
inventory, each use case classified by risk (7.d). The inventory is reviewed for
continuance of approval on a set cadence (recommend quarterly) within an existing command
forum rather than a new committee. *[COMMAND DECISION — name the forum; set the cadence.]*
...
workflow that cannot be reverted to a manual procedure quickly. Production never depends
on AI-system availability. *[MAP TO COMMAND — set the manual-fallback standard, e.g.,
revertible within one working hour.]*
...
basis (Section 2), the approved Impact Level (Section 4), the governance forum and cadence
(Section 8.g), data residency and host-nation cryptographic compliance, and the maturity
target. *[COMMAND DECISION block.]*
